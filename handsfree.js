...
handsfree.use('fingerKeys', ({
    ...
    if (hands.pinchState[0][3] === 'released') {
        releaseKey(1)
    }

    // Left ring
    if (hands.pinchState[0][2] === 'start') {
        pinchKey(2)
    }
    if (hands.pinchState[0][2] === 'released') {
        releaseKey(2)
    }
    ...
    if (hands.pinchState[0][1] === 'released') {
        releaseKey(3)
    }
    ...
    if (hands.pinchState[1][0] === 'released') {
        releaseKey(5)
    }
    ...
    if (hands.pinchState[1][1] === 'released') {
        releaseKey(6)
    }

    // Right ring
    if (hands.pinchState[1][2] === 'start') {
        pinchKey(7)
    }
    ...
    // Right pinky
    if (hands.pinchState[1][3] === 'start') {
        pinchKey(8)
    }
    if (hands.pinchState[1][3] === 'released') {
        releaseKey(8)
    }
})
